        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length >= 4) {
                    ...
                    // Normalize mode by trimming, lowercasing, and removing spaces
                    String mode = parts[2].trim().toLowerCase().replaceAll("\\s+", "");
                    int amount = Integer.parseInt(parts[3].trim());

                    if (pin.equals(inputPin)) {
                        pinFound = true;

                        // Display original mode with spaces and casing preserved
                        statement.append(date).append("&nbsp;&nbsp;&nbsp;")
                                 .append(parts[2].trim())
                                 .append("&nbsp;&nbsp;&nbsp;Rs ")
                                 .append(amount).append("<br><br>");

                        // Update balance based on transaction mode
                        if (mode.contains("deposit")) {
                            balance += amount;
                        } else if (mode.contains("withdraw") || mode.contains("fastcash")) {
                            balance -= amount;
                        }
                    }
                }
